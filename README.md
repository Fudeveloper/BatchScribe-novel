...
**强烈推荐使用：`gemini-2.0-flash-exp`**
- 性能优异，生成质量高
- 速度快，成本低
- API Key获取：[aiapi.space](https://aiapi.space) 注册即可

**其他推荐模型：**
- `gemini-2.0-flash-thinking-exp` - 思维链模型，逻辑性强
- `claude-3-5-sonnet-20241022` - 文学性强，适合创意写作
- `gpt-4o` - OpenAI最新模型，综合能力强

### 使用步骤

1. **配置API密钥**：首次启动程序时，输入从 [aiapi.space](https://aiapi.space) 获取的API密钥
2. **选择模型**：推荐使用 `gemini-2.0-flash-exp` 模型
...
| 模型         | 使用的AI模型         | gemini-2.0-flash-exp     | gemini-2.0-flash |
| 语言         | 生成小说的语言       | 中文                      | 中文             |
| 小说类型     | 生成的小说类型       | 根据需求选择              | 奇幻冒险         |
| 目标字数     | 生成小说的字数       | 20000-50000               | 20000            |
| 并行数       | 同时生成的小说数量   | 2-3（避免API限制）        | 3                |
